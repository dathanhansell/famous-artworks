...
const getArtworksByMuseum = async (req, res) => {
    ...
    try {
        const artworks = await joinTables('artworks', 'museums', 'belongs_to', 'museum_id', 'artwork_id', museumId);
        res.send(artworks);
    } catch (err) {
        res.status(500).send(err.message);
    }
...
const getArtworksByPeriod = async (req, res) => {
    ...
    try {
        const artworks = await joinTables('artworks', 'art_periods', 'included_in', 'art_period_id', 'artwork_id', periodId);
        res.send(artworks);
    } catch (err) {
        res.status(500).send(err.message);
    }
...
const getArtworksByStyle = async (req, res) => {
    ...
    try {
        const artworks = await joinTables('artworks', 'art_styles', 'falls_under', 'art_style_id', 'artwork_id', styleId);
        res.send(artworks);
    } catch (err) {
        res.status(500).send(err.message);
    }
...
const getArtworksByCollector = async (req, res) => {
    ...
    try {
        const artworks = await joinTables('artworks', 'collectors', 'owned_by', 'collector_id', 'artwork_id', collectorId);
        res.send(artworks);
    } catch (err) {
        res.status(500).send(err.message);
    }
